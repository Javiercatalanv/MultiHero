...
@export var escena_personaje_c: PackedScene
...
func cambiar_personaje():
	...
	match indice_personaje:
		...
		2:
			personaje_actual = escena_personaje_c.instantiate()
	
	indice_personaje = (indice_personaje + 1) % 3
...
func _input(event):
	...
	if event.is_action_pressed("cambioPersonaje") and puedeCambiar:
		cambiar_personaje()
